      ...
      <v-form>
        <v-card>
          <v-card-title class="headline blue darken-4">
            <span class="headline white--text">编辑类别</span>
          </v-card-title>
          <v-card-text>
            <v-container grid-list-md>
              <v-layout wrap>
                ...
                <v-flex xs12 sm6 md4 lg3>
                  <v-text-field v-model="formData.old_category" label="原分类" disabled></v-text-field>
                </v-flex>
                <v-spacer/>
                <v-flex xs12 sm6 md4 lg3>
                  <v-text-field v-model="formData.new_category" label="新分类" required></v-text-field>
                </v-flex>
                <v-spacer/>
                <v-flex xs12 sm6 md4 lg3>
                  <v-switch
          ...
          <v-card-actions>
            ...
            <v-btn color="blue darken-1 white--text" @click="$_submit(formData)" :disabled="formData.new_category===''">保存</v-btn>
          </v-card-actions>
        </v-card>
      </v-form>
    </v-dialog>
